...
Notice in the code above support for custom formatters via a template (in the style of Mustache templates). YUI DataTable was a great component, certainly lacking some features 
...
We were inspired 🙏 by AG Grid, seeing the breadth of features it offers and its expansive growth. It is a feat of engineering and shows how much the browser can be pushed by extensive use of virtualization. After vanilla JavaScript and Angular versions of AG Grid, a React version was finally released. It was a step in the right direction, even though it was a thin wrapper around React, with all the renderers and API still being imperative and not feeling like the best fit inside a React app. A few years later, AG Grid finally released a `reactUI` version, with tighter integration with React and a more declarative API ❤️.
