...
const rowStyle = (param: { rowInfo: any }) => {
  const { rowInfo } = param;
...
const ButtonsArea = defineComponent({
  setup() {
    const context = useDataSourceContext<Employee>();
    const dataSourceApi = context?.dataSourceApi;
    ...
    return () => [
      h(
        'button',
        { type: 'button', onClick: removeRowsByPrimaryKey },
        'Click Me to removeRowsByPrimaryKey!',
      ),
      h(
        'button',
        { type: 'button', onClick: removeRowsByDataRow },
