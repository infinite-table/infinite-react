...
const groupBy = [{ field: 'country' as const }, { field: 'stack' as const }];
const pivotBy = [
  {
...
const showSeparatePivotColumnForSingleAggregation = ref(true);
const totalColumnVisible = ref(true);
const useCanDesignPivot = ref(false);

const pivot = computed(() => {
  return (pivotBy as any[]).concat(
    useCanDesignPivot.value ? [{ field: 'canDesign' }] : [],
  );
});
</script>
...
<template>
  ...
  <button
    ...
    @click="totalColumnVisible = !totalColumnVisible"
  >
    Toggle total column - {{ `${totalColumnVisible}` }}
  </button>
  <button
    data-name="toggle-pivot-col"
    @click="useCanDesignPivot = !useCanDesignPivot"
  >
