...
import { mapListElements, withBrain } from '../testUtils/listUtils';
...
export default test.describe.parallel('RowListWithExternalScrolling', () => {
  ...
  test('should correctly render on scroll', async ({ page }) => {
    ...
    let elements = await mapListElements((el) => el.textContent, undefined, {
      page,
    });
    ...
    await page.waitForTimeout(30);
    elements = await mapListElements((el) => el.textContent, undefined, {
      page,
    });
    expect(elements).toEqual([
      'row 4',
